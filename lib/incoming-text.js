...
export async function textMessageHandler(msg) {
  try {
    ...
    chrome.alarms.create(`intext-cleanup-${msg.id}`, {
    ...
    const messageQueue = (await getSessionObject("intext-queue")) || [];
    if (!messageQueue.includes(msg.id)) {
      messageQueue.push(msg.id);
      await setSessionObject("intext-queue", messageQueue);
    }
  } catch (e) {
...
async function displayInText(msgId) {
  try {
    ...
    // Create a session object to get the message id of the closed popup in
    // the onRemoved listener.
    await setSessionObject(`popup-${popup.id}`, msgId);
  } catch (e) {
    if (DEBUG) console.error(e);
  }
}
