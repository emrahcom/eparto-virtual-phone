...
import { getByKey, getSessionObject, setSessionObject } from "./common.js";
...
export async function startOutCall(call) {
  try {
    ...
    chrome.alarms.create(`outcall-cleanup-${call.id}`, {
    ...
    await setSessionObject(`contact-${call.contact_id}`, call.id);

    // Save the call object into the local storage.
    await setSessionObject(`outcall-${call.id}`, call);

    // Trigger ring loop.
    chrome.alarms.create(`outcall-ring-${call.id}`, { delayInMinutes: 0.02 });
  } catch (e) {
...
export async function cleanupOutCall(callId) {
  try {
    ...
    const call = await getSessionObject(`outcall-${callId}`);
    if (!call) return;
...
export async function ringOutCall(callId) {
  try {
    ...
    // Get the active call.
    const activeCall = await getSessionObject(`contact-${call.contact_id}`);
